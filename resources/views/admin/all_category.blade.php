    ...
    <div class="row w3-res-tb">
      ...
      <div class="col-sm-3">
        <div class="input-group">
          ...
          <form role="form" action="{{URL::to('/search-cate')}}" method="post">
            {{ csrf_field() }}
      ...
      <table class="table table-striped b-t b-light">
        ...
        @foreach ($cate as $key => $cate_pro)
          ...
          <td>{{ $cate_pro->cate_name }}</td>
            <td><span class="text-ellipsis">{{$cate_pro->cate_info}}</span></td>
            <td><span class="text-ellipsis"><?php
              ...
              if ($cate_pro->cate_status == 0) { ?>
                <a href="{{URL::to('/cate-down/'.$cate_pro->id)}}"><span id="style-fa-up" class="fa fa-check"></span></a>
              <?php } else { ?>
                <a href="{{URL::to('/cate-up/'.$cate_pro->id)}}"><span id="style-fa-down" class="fa fa-check"></span></a>
             <?php }  ?>

            </span></td>
            <td>
              <a href="{{URL::to('/cate-edit/'.$cate_pro->id)}}" class="active" ui-toggle-class=""><i id="fa-style" class="fa fa-pencil-square-o text-success text-active"></i></a>
              <a onClick="return confirm('Xác nhận xóa?')" href="{{URL::to('/cate-remove/'.$cate_pro->id)}}"><br><i id="fa-style" class="fa fa-times text-danger text"></i></a>
            </td>
          </tr>
          @endforeach
        </tbody>
